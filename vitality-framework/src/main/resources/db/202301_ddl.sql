...
create TABLE VTL_MENU (
  ...
  TYPE 		                    varchar(10) NOT NULL DEFAULT 'BTN' comment '菜单类型。{ DIR=目录; MENU=菜单; BTN=按钮; IFRAME=内嵌页面；URL=外链页面；}',
  ...
  SEQ 		                    int DEFAULT 0 comment '排序。平台规定只有`home`路由的`rank`才能为`0`',
  ROUTER_NAME                   varchar(100) comment '路由名称，必须唯一。如：UserListIndex',
  ROUTER_PATH                   varchar(100) comment '路由地址。如：/vitality/system/user/index',
