...
public class CacheConfig {
    ...
    public JCacheManagerCustomizer jCacheManagerCustomizer() {
        return cacheManager -> {
            ...
            cacheManager.createCache(CacheName.DICT_LIST, config(CacheName.DICT_LIST, Duration.ofDays(7L), String.class, List.class));
            cacheManager.createCache(CacheName.DICT_DATA_LABEL, config(CacheName.DICT_DATA_LABEL, Duration.ofDays(7L), String.class, String.class));
            cacheManager.createCache(CacheName.DEPT_ID_NAME, config(CacheName.DEPT_ID_NAME, Duration.ofDays(30L), Long.class, String.class));
            cacheManager.createCache(CacheName.USER_ID_USERNAME, config(CacheName.USER_ID_USERNAME, Duration.ofDays(30L), Long.class, String.class));
            cacheManager.createCache(CacheName.USER_ID_NICKNAME, config(CacheName.USER_ID_NICKNAME, Duration.ofDays(7L), Long.class, String.class));
            cacheManager.createCache(CacheName.CAPTCHA, config(CacheName.CAPTCHA, Duration.ofMinutes(1L), String.class, ICaptcha.class));
            cacheManager.createCache(CacheName.REPEAT_SUBMIT, config(CacheName.REPEAT_SUBMIT, Duration.ofSeconds(10L), String.class, Long.class));
        };
    }
     ...
     * 默认缓存配置属性
     */
    public static <K, V> javax.cache.configuration.Configuration<K, V> config(String cacheName, Duration duration, Class<K> keyType, Class<V> valueType) {
        CacheConfigurationBuilder<K, V> builder = CacheConfigurationBuilder.newCacheConfigurationBuilder(
                ...
                // 缓存监听器
                .withService(cacheEventListener(cacheName));
    ...
    private static CacheEventListenerConfiguration<?> cacheEventListener(String cacheName) {
        return CacheEventListenerConfigurationBuilder
                .newEventListenerConfiguration(
                        new CustomCacheEventListener(cacheName), EventType.EVICTED, EventType.EXPIRED, EventType.REMOVED, EventType.CREATED, EventType.UPDATED)
                .asynchronous()
                .unordered()
                .build();
    }
