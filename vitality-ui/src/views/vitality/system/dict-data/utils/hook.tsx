...
import type { FormItemProps, DictDataVO } from "../utils/types";
...
export function useDictData(tableRef: Ref, dictCode: string) {
  const form = reactive<DictDataVO>({});
  const curRow = ref();
  const formRef = ref();
  const dataList = ref([]);
  ...
  async function onSearch() {
    ...
    const page = await getDictDataPage(toRaw(form));
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(valid => {
          if (valid) {
            ...
            // 表单规则校验通过
            if (curData.id) {
              updateDictData(curData).then(r => {
                if (r.code === 200) {
                  chores();
                }
              });
            } else {
              createDictData(curData).then(r => {
                if (r.code === 200) {
                  chores();
                }
              });
