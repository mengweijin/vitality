...
import type { FormProps, PostVO } from "../utils/types";
...
export function usePost(tableRef: Ref) {
  ...
  function beforeDisabledChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        updatePost({ id: row.id, disabled: disabled } as FormProps).then(() => {
  ...
  function openEditDialog(title = "新增", row?: PostVO) {
    addDialog({
      title: `${title}`,
      props: {
        data: {
      ...
      beforeSure: (done, { options }) => {
        ...
        formRef.value.getRef().validate(valid => {
          if (valid) {
            // 表单规则校验通过
            if (curData.id) {
              updatePost(curData).then(() => {
                chores();
              });
            } else {
              createPost(curData).then(() => {
                chores();
              });
            }
          }
  ...
  function openDetailDialog(title = "详情", row?: PostVO) {
    addDialog({
      title: `${title}`,
      props: {
        data: row
